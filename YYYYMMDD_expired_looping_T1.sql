...
/* assign row number based on the latest updated loan for the customer*/ 
        ...
        WHERE  a.row_id = 1 
               ...
               AND Trunc( From_tz(Cast(a.status_date AS TIMESTAMP), 'utc')) <= Trunc(CURRENT_DATE) - 154
               AND ( Mod( ( ( Trunc(CURRENT_DATE) - Trunc( From_tz(Cast(a.status_date AS TIMESTAMP), 'utc') ) ) - 154 ), 140) >= 1 
                     AND Mod( ( ( Trunc(CURRENT_DATE) - Trunc( From_tz(Cast(a.status_date AS TIMESTAMP), 'utc') ) ) - 154 ), 140) < 14 ) 
